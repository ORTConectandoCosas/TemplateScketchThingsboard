...
// includes libraries for comunicación and proyect
...
//  config thingsboard device data
...
/*Define topics.
 * telemetry - used to send data to server (telemetry)
 * request - used to send and receive commnads from server
 * attributes - used to send and receive attribute to/from server
 */
char telemetryTopic[] = "v1/devices/me/telemetry";
char requestTopic[] = "v1/devices/me/rpc/request/+";  //RPC 
...
void setup()
{
  ...
  // config MQTT callbacks
  client.setCallback(on_message);
   
  lastSend = 0; // aux variable to control send delay
...
void on_message(const char* topic, byte* payload, unsigned int length) 
{
  ...
  // Decode JSON request
  // Payload Format {"method": value, "params"" value}
